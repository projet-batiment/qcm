...
class Editeur(Frame):
    def __init__(self, parent, page_callback, titre="Énoncé de la question", obligatoire=True, points=1):
        ...
        self.type_selector = Combobox(self.haut, textvariable=self.type_var, values=["Choix Unique", "Choix Multiple"],state="readonly",width=15)
        self.type_selector.pack(side=LEFT, padx=10)
        self.type_selector.bind("<<ComboboxSelected>>", lambda e: page_callback(CallbackCommand.CHANGE_TYPE, self))

        self.points_entry = Spinbox(self.haut, textvariable=self.points_var, from_=0, to=100, width=4)
        ...
        self.btn_down = Button(self.bas, text=" 🠋 ", command=lambda: page_callback(CallbackCommand.MOVE_DOWN, self))
        self.btn_down.pack(side=RIGHT, padx=2)

        self.btn_up = Button(self.bas, text=" 🠉 ", command=lambda: page_callback(CallbackCommand.MOVE_UP, self))
